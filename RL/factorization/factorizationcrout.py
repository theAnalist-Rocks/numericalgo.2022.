...
def MatrixSuperior(A, b):
    ...
    while cpt != n:
        ...
        sum_right = A[cpt][ans_index + 1: m: 1] * answers[0][ans_index+1:m].T  # ils auront la même taille donc c'est bon
        if A[cpt][ans_index] != 0:
            x = (b[0][cpt] - sum_right.sum()) / A[cpt][ans_index]
...
# L, R = set_start_LU(np.array([[1, 2, 3], [3, 2, 1], [2, 1, 3]], dtype=float))
# print(L, "\n", R)
# print('Produit:\n', np.dot(L, R))
